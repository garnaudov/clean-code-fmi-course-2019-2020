...
export default class Pawn extends Piece {
  ...
  movePossible(src, dest, isDestEnemyOccupied) {

    if (this.player === 1) {
      if ((dest === src - 8 && !isDestEnemyOccupied) || (dest === src - 16 && this.initPos[1].indexOf(src) !== -1)) {
        return true;
      }
      ...
      if ((dest === src + 8 && !isDestEnemyOccupied) || (dest === src + 16 && this.initPos[2].indexOf(src) !== -1)) {
        return true;
      }
      else if (isDestEnemyOccupied && (dest === src + 9 || dest === src + 7)) {
        return true;
